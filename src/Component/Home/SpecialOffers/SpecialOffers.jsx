...
const SpecialOffers = () => {
  ...
  useEffect(() => {
    if (data?.products) {
      ...
      const onSaleProducts = data.products.filter(
        (product) => product.additionalInfo.isOnSale
      );
  ...
  return (
    <div
      className={`container mx-auto ${isDarkMode ? "bg-gray-900" : "bg-white"}`}
    >
          ...
          >
            {products.map((product) => (
              <SwiperSlide key={product._id}>
                <div
                  className={`w-full flex flex-col items-center justify-center mb-10 p-4 group cursor-pointer transition duration-300 ease-in-out hover:shadow-lg ${
                    isDarkMode
                      ? "hover:bg-gray-800 bg-gray-800"
                      : "hover:bg-white"
                  } rounded-lg lg:rounded-lg mb-5`}
                >
                  <Link
                  ...
                  >
                    ...
                    >
                      <h2
                        className={`text-base font-semibold mb-2 mt-10 line-clamp-2 h-12 ${
