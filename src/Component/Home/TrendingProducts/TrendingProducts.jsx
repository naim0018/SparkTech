  ...
  return (
    <div className="w-full flex flex-col items-center justify-center p-4 rounded-lg group cursor-pointer">
      ...
      <div className="flex items-center my-2">
        ...
        <span className="text-gray-600 ml-2 text-xs sm:text-sm">{product.rating.count}</span>
      </div>
      ...
      <div className="flex items-center justify-between w-full px-2">
        <div className="flex items-center mb-2">
          ...
          {product.price.discounted && (
            <span className="text-gray-500 line-through ml-2 text-xs sm:text-sm">${product.price.discounted}</span>
          )}
        </div>
...
const TrendingProducts = () => {
  ...
  const trendingProducts = data?.data?.filter(product => product.isFeatured || product.isOnSale).slice(0, 8) || []
